...
import { User, Video, Briefcase, Calendar, MessageSquare, Moon, Sun, X } from 'lucide-react';

const Sidebar = ({ toggleTheme, theme, isOpen, closeMenu }) => {
  const navItems = [
  ...
  return (
    <>
      ...
      <aside className={`glass-panel sidebar-container ${isOpen ? 'open' : ''}`} style={{ width: '280px', height: '100vh', borderRadius: '0 24px 24px 0', borderLeft: 'none', top: 0, padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column', background: 'var(--color-bg)', borderRight: '1px solid var(--glass-border)' }}>
        <div style={{ marginBottom: '3rem', textAlign: 'center', position: 'relative' }}>
          ...
          <button className="mobile-menu-btn" onClick={closeMenu} style={{ position: 'absolute', right: '-0.5rem', top: '-0.5rem', background: 'none', border: 'none', cursor: 'pointer' }}>
